...
class TodoList
  ...
  def create_file
    unless File.exist?("./todo_folder/#{@file_name}")
  ...
  def read_todo
    ...
    tasks.each { |task_line| add_task(Task.new(task_line)) }
  end

  def write_todo
    File.open("./todo_folder/#{@file_name}", 'w') do |f|
      @tasks.each { |t| f.puts(t.line_for_file) } if @tasks
    end
  end
  ...
  def remove_task(index)
    @tasks.delete_at(index) if get_by_index(index)
  end
